# Binary Cross Entropy Loss + Dice Loss + Topological Pooling Loss
def TopologicalPoolingLoss(y_hat,y, device='cuda'):
    ...
    def loss_per_batch(y_hat,y):
        ...
        for k_size in kernel_sizes:
            ...
            diff=torch.abs(((G_topo_k)-(P_topo_k))) #Get how many pixels do not match Ground Truth in the 3 planes
            loss_k_mean = torch.mean(diff)  #Average of the errors for each size of k
            loss_values.append(loss_k_mean)
        # Average over all kernel sizes
    ...
    #Computing Topological Pooling Loss
    # Init dice score for batch
    ...
    # Return the mean Loss over the given batch
    l_topo_batch = l_topo_val / (pair_idx + 1)
    #Averaging the Loss Functions
    weight_1=0.4
    weight_2=0.3
    weight_3=0.3
