...
def dice_coeff_batch(batch_bn_mask, batch_true_bn_mask, device = 'cuda'):
    ...
    def single_dice_coeff(input_bn_mask, true_bn_mask):
        ...
        # inter_mask = torch.sum(input_bn_mask * true_bn_mask)
...
class EarlyStopping:
    """Early stops the training if validation loss doesn't improve after a given patience."""
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint.pt',path_v2='checkpoint.pt', trace_func=print):
        """
        Args:
            patience (int): How long to wait after last time validation loss improved.
                            Default: 7
        ...
        self.trace_func = trace_func
    def __call__(self, val_loss, model,epoch, op, sche):

        score = -val_loss

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, epoch, op, sche)
        elif score < self.best_score + self.delta:
            self.counter += 1
            self.trace_func(f'EarlyStopping counter: {self.counter} out of {self.patience}')
            if self.counter >= self.patience:
    ...
    def save_checkpoint(self, val_loss, model, epoch, optim, sche):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            self.trace_func(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        
        torch.save({'model': model.state_dict(),'epoch': epoch, 'optim': optim.state_dict(), 'scheduler': sche.state_dict()}, self.path_v2)
        torch.save(model.state_dict(), self.path)
        self.val_loss_min = val_loss
